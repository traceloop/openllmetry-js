...
// =============================================================================
//  Maps a single Google GenAI SDK Part to its OTel-compliant part object.
//
//  Google GenAI Parts are union-typed by which property is present (no `type`
//  discriminator field). The mapper inspects known properties in priority order:
//
//   { text }                 → TextPart       (with thought=true → ReasoningPart)
//   { inlineData }           → BlobPart       { modality inferred from mime_type }
//   { fileData }             → UriPart        { modality inferred from mime_type }
//   { functionCall }         → ToolCallRequestPart
//   { functionResponse }     → ToolCallResponsePart
//   { executableCode }       → GenericPart    { type: "executable_code" }
//   { codeExecutionResult }  → GenericPart    { type: "code_execution_result" }
//   <unknown>                → GenericPart
 ...
 * Used by formatInputMessages, formatOutputMessage, and formatSystemInstructions.
 */
export function mapGoogleGenAIContentBlock(block: any): object {
  if (typeof block === "string") {
    return { type: "text", content: block };
  }

  // Text — check thought flag for reasoning content
  if (block.text !== undefined) {
    if (block.thought) {
      return { type: "reasoning", content: block.text };
    }
    return { type: "text", content: block.text };
  }
  ...
  if (block.inlineData) {
    const modality = inferModality(block.inlineData.mimeType);
    return {
      type: "blob",
      ...(modality && { modality }),
      mime_type: block.inlineData.mimeType,
  ...
  if (block.fileData) {
    const modality = inferModality(block.fileData.mimeType);
    return {
      type: "uri",
      ...(modality && { modality }),
      mime_type: block.fileData.mimeType,
      uri: block.fileData.fileUri,
    };
  }

  // Function call (tool use requested by model)
  ...
  // Function response (tool result sent back to model)
  ...
  // Executable code (code generated by the model)
  ...
  // Unknown part — preserve as GenericPart
  return { type: "unknown", ...block };
}
